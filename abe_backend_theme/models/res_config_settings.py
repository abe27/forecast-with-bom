...
class ResConfigSettings(models.TransientModel):
    ...
    app_bar_color = fields.Char(string='Appbar color',
                                config_parameter='abe_backend_theme.'
                                                 'appbar_color',
                                default='#000000',
                                help="App bar color")
    ...
    def config_color_settings(self):
        ...
        colors = {
            ...
            'appbar_color': self.env[
              ...
              'abe_backend_theme.appbar_color'),
            'primary_accent': self.env[
              'ir.config_parameter'].sudo().get_param(
              'abe_backend_theme.primary_accent_color'),
            'secondary_color': self.env[
              'ir.config_parameter'].sudo().get_param(
              'abe_backend_theme.secondary_color'),
            'kanban_bg_color': self.env[
              'ir.config_parameter'].sudo().get_param(
              'abe_backend_theme.kanban_bg_color'),
            'primary_hover': self.env[
              'ir.config_parameter'].sudo().get_param(
              'abe_backend_theme.primary_hover'),
            'light_hover': self.env[
              'ir.config_parameter'].sudo().get_param(
              'abe_backend_theme.light_hover'),
            'appbar_text': self.env[
              'ir.config_parameter'].sudo().get_param(
              'abe_backend_theme.appbar_text'),
            'secondary_hover': self.env[
              'ir.config_parameter'].sudo().get_param(
              'abe_backend_theme.secondary_hover')
        }
        return colors
